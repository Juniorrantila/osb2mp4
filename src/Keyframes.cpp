...
template <typename T>
T Keyframe::keyframeValueAt(const std::vector<Keyframe<T>>& keyframes, double time)
{
...
template <class T>
std::pair<T, T> Keyframe::keyframeValueAt(const std::pair<std::vector<Keyframe<T>>, std::vector<Keyframe<T>>>& keyframes, double time)
{
...
void Keyframe::addKeyframe(Selector W, std::vector<Keyframe<T>>& keyframes, double time, V value, bool alt, Easing easing, double interpolationOffset = std::numeric_limits<double>::infinity())
{
...
void Keyframe::addKeyframe(Selector W, std::vector<Keyframe<T>>& keyframes, double time, ParameterType value, bool alt, Easing easing, double interpolationOffset = std::numeric_limits<double>::infinity())
{
    keyframes.push_back(Keyframe<T>(time, alt, Easing::Step, interpolationOffset));
}

template <typename T, typename V, typename Selector>
void Keyframe::generateKeyframes(std::vector<Keyframe<T>>& keyframes, const std::vector<std::unique_ptr<Event<V>>>& events, const std::vector<std::tuple<double, double, int>>& activations, Selector W)
{
...
R Keyframes::generateKeyframesForEvent(const std::vector<std::unique_ptr<IEvent>>& events, std::pair<double, double> coordinates, const std::vector<std::tuple<double, double, int>>& activations)
{
