...
std::pair<unsigned, unsigned> Storyboard::GetResolution() const
{
    return resolution;
}

std::pair<double, double> Storyboard::GetActiveTime() const
{
    return activetime;
}

double Storyboard::GetAudioDuration() const
{
    return audioDuration;
}

double Storyboard::GetAudioLeadIn() const
{
    return audioLeadIn;
}
...
void Storyboard::SetPixel(cv::MatIterator_<cv::Vec<uint8_t, 3>> imageStart, int width, const int x, int y, cv::Vec<uint8_t, 3> pixel) const
{
    *(imageStart + y * width + x) = pixel;
}

cv::Vec<uint8_t, 3> Storyboard::GetPixel(const cv::MatIterator_<cv::Vec<uint8_t, 3>> imageStart, int width, int x, int y) const
{
...
cv::Vec2f Storyboard::GetOriginVector(const Origin origin, const int width, const int height) const
{
    switch (origin)
    {
